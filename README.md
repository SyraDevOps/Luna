...
# Análise do Sistema AutoML na LunaGPT

Analisando os logs do treinamento, posso confirmar que o sistema AutoML está funcionando corretamente, mas com algumas considerações importantes:

## O que está funcionando:
- O sistema inicia corretamente com `lunagpt_automl_luna` como nome do estudo
- Os trials estão sendo executados sequencialmente, com diferentes parâmetros
- O wandb está registrando métricas para cada trial
- O ajuste dinâmico de uso de memória está ativado

## Pontos de atenção:
- Alguns trials estão terminando com valor `inf`, indicando instabilidade durante o treinamento
- O sistema ajusta constantemente o batch size e gradient accumulation steps devido ao uso crítico de memória (>90%)
- Há um erro de indexação no cálculo de perda causal que precisa ser tratado
- Como não foi definido um limite claro para o número total de trials, o sistema pode continuar executando por tempo excessivo

O comportamento observado é esperado para o AutoML em hardware com recursos limitados. O sistema está tentando otimizar os hiperparâmetros enquanto gerencia as restrições de memória.

---

# Seção para o README: Otimização de Hiperparâmetros com AutoML

```markdown
## Otimização Automática de Hiperparâmetros

O LunaGPT 2.6v inclui um sistema de otimização automática de hiperparâmetros (AutoML) que pode descobrir as melhores configurações para seu modelo com base nos seus dados específicos.

### Uso Básico

Para treinar um modelo com otimização automática de hiperparâmetros:

```bash
python main.py train --model seu_modelo --train_data "dados/treino/*.json" --automl
```

### Parâmetros Avançados

```bash
python main.py train --model seu_modelo --train_data "dados/treino/*.json" --automl --automl_trials 10
```

| Parâmetro | Descrição |
|-----------|-----------|
| `--automl` | Ativa a otimização de hiperparâmetros |
| `--automl_trials N` | Define o número de tentativas de otimização (padrão: 10) |
| `--dynamic_hp_opt` | Ativa otimização dinâmica durante o treinamento (alternativa mais leve) |

### Hiperparâmetros Otimizados

O sistema otimiza automaticamente:
- Taxa de aprendizado (learning rate)
- Peso de decaimento (weight decay) 
- Tamanho de batch
- Passos de acumulação de gradiente
- Taxa de warmup
- Taxa de dropout
- Taxa de dropout de atenção

### Requisitos de Hardware

A otimização de hiperparâmetros requer recursos computacionais significativos:
- Para melhores resultados, use GPU com pelo menos 8GB de VRAM
- Em sistemas com recursos limitados, reduza `--automl_trials` para 5 ou menos
- Em hardware de baixo desempenho, considere usar `--dynamic_hp_opt` ao invés de `--automl`

### Resultados e Visualizações

Os resultados da otimização são salvos em:
- `models/seu_modelo/automl/best_hyperparams.json`: Melhores hiperparâmetros encontrados
- `models/seu_modelo/automl/trial_*`: Artefatos de cada tentativa individual
- Wandb: Gráficos e métricas detalhadas se o W&B estiver configurado

Para visualizar os resultados após a conclusão:

```bash
python -m src.optimization.automl_visualization models/seu_modelo/automl/lunagpt_automl_seu_modelo_*.json
```
```

Esta documentação oferece aos usuários uma visão clara de como utilizar a funcionalidade de AutoML em diferentes cenários, incluindo dicas para gerenciar o uso em hardware limitado.Esta documentação oferece aos usuários uma visão clara de como utilizar a funcionalidade de AutoML em diferentes cenários, incluindo dicas para gerenciar o uso em hardware limitado.