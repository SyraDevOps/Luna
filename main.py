...
def ensure_data_directories():
    ...
    directories = ["data/train", "data/valid", "models"]
    for directory in directories:
        os.makedirs(directory, exist_ok=True)
        logger.info(f"Diretório verificado/criado: {directory}")

    # Criar um arquivo de exemplo em data/train se estiver vazio
    train_sample_path = os.path.join("data/train", "sample.txt")
    if not os.listdir("data/train"):
        with open(train_sample_path, "w", encoding="utf-8") as f:
            f.write("Este é um exemplo de texto para treinar o modelo do zero.\n")
        logger.info(f"Arquivo de exemplo criado em: {train_sample_path}")

    # Criar um arquivo de exemplo em data/valid se estiver vazio
    valid_sample_path = os.path.join("data/valid", "sample.txt")
    if not os.listdir("data/valid"):
        with open(valid_sample_path, "w", encoding="utf-8") as f:
            f.write("Este é um exemplo de texto para validar o modelo.\n")
        logger.info(f"Arquivo de exemplo criado em: {valid_sample_path}")
...
def train_model(args, config):
    ...
    # Configurar otimização para hardware leve
    if hasattr(args, 'disable_lightweight') and args.disable_lightweight:
        config.model.use_lightweight_mode = False
        logger.info("Otimização para hardware leve desativada")
    else:
        config.model.use_lightweight_mode = True
        logger.info("Otimização para hardware leve ativada (padrão)")
    ...
    # Garantir pelo menos 5 épocas para testes de treinamento
    if not args.epochs or args.epochs < 5:
        config.training.num_train_epochs = 5
        logger.info("Número mínimo de épocas definido para 5 para teste completo")
    else:
        config.training.num_train_epochs = args.epochs
        logger.info(f"Número de épocas definido para: {args.epochs}")
    ...
    if not train_data:
        logger.error("Nenhum dado de treinamento encontrado. Adicione arquivos suportados nas pastas de dados.")
        return
    
    logger.info(f"Dados carregados: {len(train_data)} exemplos de treinamento, {len(valid_data)} exemplos de validação")
    
    # Inicializar trainer
    trainer = LunaTrainer(model_name, config)
    
    # Treinar modelo
    result = trainer.train_supervised(train_data, valid_data)
    
    if result['success']:
        logger.info(f"Treinamento do modelo {model_name} concluído com sucesso!")
        if 'metrics' in result and result['metrics']:
            last_metrics = result['metrics'][-1]
            if 'eval_loss' in last_metrics:
                logger.info(f"Perda final de validação: {last_metrics['eval_loss']:.4f}")
    else:
        logger.error(f"Falha no treinamento: {result.get('error', 'Erro desconhecido')}")
...
def parse_args():
    ...
    # Comando 'create'
    parser_create = subparsers.add_parser("create", help="Cria o modelo e ambiente")
    parser_create.add_argument("--name", type=str, required=True, help="Nome do modelo")
    parser_create.add_argument("--train_data", nargs="+", help="Padrões de dados de treinamento")
    
    # Comando 'train'
    parser_train = subparsers.add_parser("train", help="Realiza treinamento")
    parser_train.add_argument("--model", type=str, required=True, help="Nome do modelo a treinar")
    parser_train.add_argument("--epochs", type=int, help="Número de épocas")
    parser_train.add_argument("--train_data", nargs="+", help="Padrões de dados de treinamento")
    parser_train.add_argument("--valid_data", nargs="+", help="Padrões de dados de validação")
    ...
    for cmd_parser in [parser_train, parser_chat, parser_refine]:
