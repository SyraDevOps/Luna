...
def train_model(args, config):
    ...
    # Definir número de épocas se especificado
    num_train_epochs = None
    if hasattr(args, 'epochs') and args.epochs is not None:
        num_train_epochs = args.epochs
        logger.info(f"Número de épocas definido pelo usuário: {num_train_epochs}")
    ...
    # Treinar modelo
    if args.curriculum:
        # Curriculum learning com número personalizado de épocas
        result = trainer.train_with_curriculum(train_data, valid_data, use_wandb=args.use_wandb)
    else:
        # Treinamento normal com número personalizado de épocas
        result = trainer.train_supervised(train_data, valid_data, use_wandb=args.use_wandb, num_train_epochs=num_train_epochs)
    ...
    if result['success']:
        ...
        if 'metrics' in result and result['metrics']:
            ...
            if 'eval_perplexity' in last_metrics:
                logger.info(f"Perplexidade final: {last_metrics['eval_perplexity']:.4f}")
    else:
...
def parse_args():
    ...
    parser_train.add_argument("--use_wandb", action="store_true", help="Ativar integração com Wandb para monitoramento")
    parser_train.add_argument("--curriculum", action="store_true", help="Usar curriculum learning")
    ...
    for cmd_parser in [parser_train, parser_chat, parser_refine, memory_parser, tokens_parser]:
