...
def load_data_from_patterns(patterns, auto_split=True, split_ratio=0.2):
    """Carrega dados de múltiplos padrões de arquivo"""
    all_data = []
    file_count = 0
    ...
    if not all_data:
        if file_count > 0:
            logger.warning("Arquivos encontrados, mas nenhum dado extraído. Verifique os formatos.")
        else:
            logger.warning("Nenhum arquivo encontrado nos padrões especificados.")
        return [], []
    
    logger.info(f"Total de {len(all_data)} amostras carregadas")
    
    if auto_split and split_ratio > 0:
        split_idx = max(1, int(len(all_data) * (1 - split_ratio)))
