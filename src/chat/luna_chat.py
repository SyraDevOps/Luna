...
class LunaChat:
    ...
    def __init__(self, model_name: str, config, persona: str = "casual"):
        ...
        self.fallback_mode = False  # Adicionando atributo fallback_mode
    ...
    def _apply_persona_style(self, prompt):
        """Aplica o estilo de persona ao prompt."""
        persona_styles = {
            "default": "",
            "casual": "[persona: casual, amigável, informal, conversacional]",
            "tecnico": "[persona: técnico, preciso, informativo, detalhado]",
            "conciso": "[persona: conciso, direto, objetivo]",
            "academico": "[persona: acadêmico, formal, completo, referenciado]",
            "criativo": "[persona: criativo, expressivo, imaginativo]"
        }
        
        style = persona_styles.get(self.persona.lower(), "")
        if style:
            return f"{prompt}\n{style}"
        return prompt
