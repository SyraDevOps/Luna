...
class RAGRetriever:
    ...
    def __init__(self, embedding_dim: int = 768, use_faiss: bool = True, 
        ...
        # Usar memory_system se fornecido
        ...
        if memory_system is not None and memory_system.embedding_model is not None:
            self.embedding_model = memory_system.embedding_model
        else:
    ...
    def _load_default_embedding_model(self):
        """Carrega o modelo de embeddings padrão"""
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                # Tentar carregar um modelo multilíngue
                model_name = 'distiluse-base-multilingual-cased-v1'
                embedding_model = SentenceTransformers(model_name)
    ...
    def _compute_embedding(self, text: str) -> np.ndarray:
        ...
        if self.memory_system is not None:
            return self.memory_system._compute_embedding(text)
        
        # Usar a implementação original caso não tenha memory_system
    ...
    def add_documents(self, documents: List[str], metadatas: Optional[List[Dict]] = None):
        ...
        # Adicionar ao memory_system se disponível
        if self.memory_system is not None:
            for doc, meta in zip(documents, metadatas):
                self.memory_system.add_semantic_memory(doc, importance=1.0, metadata=meta)
