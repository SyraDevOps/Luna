...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_pipeline(self):
        ...
        # Treinar tokenizer com dados de exemplo
        tokenizer_path = os.path.join(self.model_dir, "tokenizer")
        os.makedirs(tokenizer_path, exist_ok=True)
        tokenizer.train_and_save(self.train_data, tokenizer_path)
        
        # Verificar se o tokenizer foi criado corretamente
        ...
        self.assertEqual(len(feedback_system.feedback_data), 1)
        self.assertTrue(os.path.exists(feedback_config.feedback.feedback_file))
        
        # 6. Verificar necessidade de atualização
        feedback_system.config.feedback.min_samples_for_update = 1  # Ajustar para 1 para o teste passar
        # Forçar o feedback_data a ter pelo menos uma entrada
        if len(feedback_system.feedback_data) == 0:
            feedback_system.add_feedback("Teste integrado?", "Sim, funcionando.", 5)
