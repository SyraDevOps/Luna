...
class LunaTrainer:
    ...
    def __init__(self, model_name, config):
        ...
        self.warmup_steps = getattr(config.training, "warmup_steps", 0)
        self.weight_decay = getattr(config.training, "weight_decay", 0.01)
        self.logging_steps = getattr(config.training, "logging_steps", 10)
        self.save_steps = getattr(config.training, "save_steps", 500)
        self.eval_steps = getattr(config.training, "eval_steps", 100)
        self.use_gradient_checkpointing = getattr(config.training, "gradient_checkpointing", False)
        self.use_fp16 = getattr(config.training, "fp16", False)
    ...
    def train_supervised(self, train_data, valid_data=None, use_wandb=False, num_train_epochs=None, output_dir=None, dynamic_hp_opt=False):
        ...
        try:
            ...
            trainer.compute_loss = lambda model, inputs, return_outputs=False, **kwargs: self._compute_causal_lm_loss_wrapper(model, inputs, return_outputs, **kwargs)
